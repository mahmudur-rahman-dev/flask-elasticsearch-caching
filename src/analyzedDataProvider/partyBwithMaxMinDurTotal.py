...
def formatDataforMaxMinDuration(df, max_or_min):
    ...
    if (not df.empty):
        ...
        if(max_or_min is "max"):
            return aggrByPartyB.nlargest(numberOfmsisdns, 'callDuration')
            # aggrByPartyB.sort_values("callDuration", ascending=False)
            # return aggrByPartyB[aggrByPartyB['callDuration'] == aggrByPartyB['callDuration'].max()]

        if(max_or_min is "min"):
            # return aggrByPartyB[aggrByPartyB['callDuration'] == aggrByPartyB['callDuration'].min()]
            return aggrByPartyB.nsmallest(numberOfmsisdns, 'callDuration')
