...
def formatData(df):
    ...
    if(not df.empty):
        df = df[[ '_source.user_id','_source.request_type', '_source.search_summary_uuid']]
        df.drop_duplicates(subset="_source.search_summary_uuid", inplace=True)
        
        df.rename(columns={
            '_source.user_id' : 'userId',
            '_source.request_type' : 'requestType',
            '_source.search_summary_uuid': 'searchSummaryUuid'
        }, inplace=True)

        app.logger.info(f'df after rename\n {df}')
        grpby = df.groupby(['requestType']).size().reset_index(name='count')

        for taskType in constant.TaskTypes:
            if taskType not in grpby['requestType'].values:
                grpby.loc[grpby.shape[0]] = [taskType, 0]
...
def buildQuery(userId,startTime,endTime):
    query = {
        ...
        "_source" : [
            'user_id','request_type', 'search_summary_uuid'
        ]
